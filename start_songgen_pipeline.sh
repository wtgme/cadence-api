...
nohup bash -c "source $CONDA_SH && cd $WORKDIR && PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True SONGGEN_LM_GPU_IDS=$LM_IDS SONGGEN_DIFF_GPU_IDS=$DIFF_IDS conda run -n musicgen --no-capture-output python -m uvicorn songgeneration_pipeline_server:app --host 0.0.0.0 --port 8888" >> "$WORKDIR/logs/pipeline_stdout.log" 2>&1 &
